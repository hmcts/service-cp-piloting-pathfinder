...
@Component
@ConditionalOnProperty(name = "auth.provider", havingValue = MockOAuthFilter.MOCK_AUTH_PROVIDER)
@Slf4j
public class MockOAuthFilter extends OncePerRequestFilter {
    public final static String MOCK_AUTH_PROVIDER = "oauth-mock";
    ...
    public MockOAuthFilter(final JWTService jwtService,
    ...
    protected void doFilterInternal(final HttpServletRequest request, final HttpServletResponse response, final FilterChain filterChain) throws ServletException, IOException {
        ...
        log.info("Mock OAuth Filter invoked");
        final String authHeader = request.getHeader(AUTHORIZATION_HEADER);

        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            log.error("MockOAuthFilter expected header {} with Bearer token not passed", AUTHORIZATION_HEADER);
    ...
    protected boolean shouldNotFilter(final HttpServletRequest request) {
        log.debug("OAuth Filter, checking should not filter {}", request.getRequestURI());
