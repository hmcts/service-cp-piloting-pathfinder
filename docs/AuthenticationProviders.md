...
2. **`oauth-mock`** - OAuth2 Resource Server with HS256 (HMAC) tokens
...
export AUTH_PROVIDER=oauth-mock
...
# application.yaml
auth:
  provider: jwt  # or oauth-mock, oauth-RS256
```
...
### 2. OAuth Mock Provider (`oauth-mock`)
...
```java
@SpringBootTest(properties = {"auth.provider=oauth-mock"})
...
### Development
- Use `jwt` or `oauth-mock` providers
...
1. Change `auth.provider` from `jwt` to `oauth-mock`
...
1. Change `auth.provider` from `oauth-mock` to `oauth-RS256`
